...
namespace Slate
{
	...
	public class ActEditor : EditorWindow
	{
		...
		public static ActEditor current;
		public static event System.Action OnStopInEditor;

		private ActSequence _cutscene;
		...
		[System.NonSerialized] private ActSequence.WrapMode editorPlaybackWrapMode = ActSequence.WrapMode.Loop;
		...
		public ActSequence cutscene
		{
			get
			{
				if (_cutscene == null)
				{
					_cutscene = EditorUtility.InstanceIDToObject(_cutsceneID) as ActSequence;
				}
				return _cutscene;
			}
			private set
		...
		public static void ShowWindow(ActSequence newCutscene)
		{
			var window = EditorWindow.GetWindow(typeof(ActEditor)) as ActEditor;
...
#pragma warning restore
		...
		void OnSelectionChange()
		{
			...
			{
				var cut = Selection.activeGameObject.GetComponent<ActSequence>();
		...
		void InitializeAll(ActSequence newCutscene)
		{
		...
		public void Play(ActSequence.WrapMode wrapMode = ActSequence.WrapMode.Loop, System.Action callback = null)
		{
			...
			if (Application.isPlaying)
			{
				...
				cutscene.Play(0, length, cutscene.defaultWrapMode, callback, ActSequence.PlayingDirection.Forwards);
		...
		public void PlayReverse()
		{
			...
			if (Application.isPlaying)
			{
				...
				cutscene.Play(0, length, cutscene.defaultWrapMode, null, ActSequence.PlayingDirection.Backwards);
				cutscene.currentTime = temp;
				return;
			}
		...
		void OnEditorUpdate()
		{
			...
			{
				if (editorPlaybackWrapMode == ActSequence.WrapMode.Once)
				{
					Stop(true);
					return;
				}
				if (editorPlaybackWrapMode == ActSequence.WrapMode.Loop)
				{
					cutscene.Sample(0);
		...
		void ShowWelcome()
		{
			...
			if (!isAboutButtonPressed)
			{
				...
				{
					InitializeAll(MyCommands.CreateCutscene());
				}
